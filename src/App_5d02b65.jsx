...
import { ShieldCheck, Globe, Activity, Search, AlertTriangle, Eye, Server, MapPin, RefreshCw, Settings, GitMerge, List, ToggleLeft, ToggleRight, Cookie } from "lucide-react";
...
// ΓöÇΓöÇ Hard-coded color palette ΓÇô immune to Chrome extension CSS-variable overrides ΓöÇΓöÇ
const C = {
  // Backgrounds
...
function App() {
  ...
  useEffect(() => {
    chrome.storage.local.get(["geminiApiKey", "blockingMode"], (res) => {
  ...
  const actionables = useMemo(() => journeyNodes.filter(n => n.trackerData || n.threatData), [journeyNodes]);
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          {viewMode === "list" && !selectedPermission && (
            <motion.div key="list" variants={pageVariants} initial="initial" animate="animate" exit="exit">
              ...
              {/* Website Hero Card */}
              {tab && (
                <motion.section layout style={{ marginBottom: 20, borderRadius: 20, background: "linear-gradient(135deg,#0f172a 0%,#1e1b4b 100%)", padding: 20, color: "#fff", boxShadow: "0 12px 40px rgba(15,23,42,0.25)", position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", right: -20, top: -20, opacity: 0.06 }}>
                    <Globe style={{ width: 140, height: 140 }} />
                  </div>
                  <div style={{ position: "relative", zIndex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 10, height: 10, borderRadius: "50%", background: telemetry?.security?.isHttps ? C.emerald : C.amber, boxShadow: `0 0 10px ${telemetry?.security?.isHttps ? C.emerald : C.amber}` }} />
                        <p style={{ fontSize: 11, color: "#94a3b8", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase" }}>Target Analysis</p>
                      </div>
                      <div style={{ background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", padding: "4px 10px", borderRadius: 8, fontSize: 10, fontWeight: 700, border: "1px solid rgba(255,255,255,0.1)", color: "#fff" }}>
                        {userActivity}
                      </div>
                    </div>
                    <h2 style={{ fontSize: 20, fontWeight: 800, margin: "0 0 0 0", color: "#fff", letterSpacing: "-0.5px" }}>{domain}</h2>
                    {telemetry && (
                      <div style={{ display: "flex", gap: 20, marginTop: 16, fontSize: 12, color: "#94a3b8", fontWeight: 500 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <Server style={{ width: 14, height: 14, color: "#818cf8" }} /> {telemetry.network.length} Requests
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <Eye style={{ width: 14, height: 14, color: C.emerald }} /> {telemetry.cookies.length} Cookies
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {telemetry ? (
                <motion.div layout>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, padding: "0 4px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <Activity style={{ width: 16, height: 16, color: C.indigo }} />
                      <h3 style={{ fontSize: 14, fontWeight: 800, color: C.textPrimary, margin: 0 }}>Tracker Controls</h3>
                    </div>
                  </div>
                            ...
                            >
                              ...
                              >
                                ...
                                <span style={{
                                  ...
                                  {isBlocked ? "≡ƒÜ½ Blocked" : (node.trackerData?.purpose || "Unclassified")}
                                </span>
                              </div>
                                  ...
                                  >
                                    <div style={{ padding: 16 }}>
                                        ...
                                        <ul style={{ fontSize: 11, color: C.textSecond, margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
                                          {node.trackerData?.fingerprinting && (
                                            <li style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                              <span style={{ color: C.indigo }}>Γ£ô</span> Fingerprinting capability
                                            </li>
                                          )}
                                          {(node.trackerData?.knownBehaviors || []).map(b => (
                                            <li key={b} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                              <span style={{ color: C.indigo }}>Γ£ô</span> {b}
                                            </li>
                                          ))}
                                        </ul>
                                      </div>
                                      ...
                                      <div style={{ paddingTop: 16, borderTop: `1px solid ${C.borderDef}` }}>
                                        ...
                                        <div style={{ background: C.cardBg, padding: 12, borderRadius: 12, border: `1px solid ${C.borderDef}`, marginBottom: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                          <div>
                                            ...
                                            <div style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, fontWeight: 700, background: recColor.bg, color: recColor.text, border: `1px solid ${recColor.border}`, display: "inline-flex", alignItems: "center", gap: 6 }}>
                                              {rec.action === "Allow" ? "≡ƒƒó Allow" : rec.action === "Review" ? "≡ƒƒí Review" : "≡ƒö┤ Block"}
                                            </div>
                                          </div>
                                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                                            <span style={{ fontSize: 10, color: C.textTertiary, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 6 }}>Status</span>
                                            <button onClick={e => { e.stopPropagation(); handleToggleBlock(node.domain, !isBlocked, "permanent"); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                                              {isBlocked
                                                ? <ToggleRight style={{ color: C.rose, width: 32, height: 32 }} />
                                                : <ToggleLeft style={{ color: C.textTertiary, width: 32, height: 32 }} />
                                              }
                                            </button>
                                          </div>
                                        </div>
                                        ...
                                        <div style={{ display: "flex", gap: 8 }}>
                                          {/* Block Session */}
                                          <button
                                            onClick={e => { e.stopPropagation(); handleToggleBlock(node.domain, !isBlocked, "session"); }}
                                            style={{
                                              flex: 1, padding: "10px 6px", borderRadius: 12, fontSize: 11, fontWeight: 700,
                                              cursor: "pointer",
                                              border: `1px solid ${isBlocked && blockType === "session" ? "#fecaca" : C.borderDef}`,
                                              background: isBlocked && blockType === "session" ? "#fff1f2" : C.surfaceBg,
                                              color: isBlocked && blockType === "session" ? "#b91c1c" : C.textSecond,
                                              display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                                            }}
                                          >
                                            {isBlocked && blockType === "session" ? "Γå⌐ Unblock" : "ΓÅ▒ Block Session"}
                                          </button>
                                          {/* Block Permanent */}
                                          <button
                                            onClick={e => { e.stopPropagation(); handleToggleBlock(node.domain, !isBlocked, "permanent"); }}
                                            style={{
                                              flex: 1, padding: "10px 6px", borderRadius: 12, fontSize: 11, fontWeight: 700,
                                              cursor: "pointer", border: "none",
                                              background: isBlocked && blockType === "permanent" ? "#dc2626" : C.indigo,
                                              color: "#ffffff",
                                              boxShadow: isBlocked && blockType === "permanent"
                                                ? "0 4px 12px rgba(220,38,38,0.35)"
                                                : "0 4px 12px rgba(79,70,229,0.35)",
                                              display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                                            }}
                                          >
                                            {isBlocked && blockType === "permanent" ? "Γå⌐ Unblock" : "≡ƒ¢í Block Always"}
                                          </button>
                                        </div>
                                      </div>
                                    </div>
                  ...
                  <div style={{ marginTop: 32, paddingTop: 20, borderTop: `1px solid ${C.borderLight}` }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      {Object.entries(telemetry.permissions || {}).map(([perm, state]) => {
                        ...
                        if (state === "granted") badge = { bg: "#fff1f2", text: "#b91c1c", border: "#fecaca" };
                        else if (state === "prompt") badge = { bg: "#fffbeb", text: "#92400e", border: "#fde68a" };
                        else if (state === "denied") badge = { bg: "#ecfdf5", text: "#065f46", border: "#a7f3d0" };
                        ...
                        return (
                          <motion.button
                            ...
                            style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", padding: 12, borderRadius: 12, background: C.cardBg, border: `1px solid ${C.borderDef}`, cursor: "pointer", textAlign: "left", boxShadow: "0 1px 4px rgba(0,0,0,0.03)" }}
                          >
                            <span style={{ fontSize: 12, fontWeight: 700, color: C.textPrimary, marginBottom: 6 }}>{formatPerm(perm)}</span>
                            <span style={{ fontSize: 9, padding: "2px 8px", borderRadius: 6, background: badge.bg, color: badge.text, border: `1px solid ${badge.border}`, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                              {state}
                            </span>
                          </motion.button>
                        );
